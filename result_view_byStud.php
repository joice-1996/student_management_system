...
<html>
    ...
    <body class="table">
	<a href="stud_home.php">Back To home</a>
	<?php
	  $id=$_SESSION['id'];
	  $qry="select batch,enrollment_id from student_details where login_id=$id";
	  $result1=mysqli_query($mysql,$qry);
	  $ro=mysqli_fetch_assoc($result1);
	  $batch=$ro['batch'];
	  $enroll=$ro['enrollment_id'];
...
<form action="" method="post">
	   ...
	   <select name="exam_name">
		   ...
		   <?php
		     $qry="select distinct exam_name from exam_notifications";
	   ...
	   <select name="exam_date">
	       <option>--select--</option>
		   <?php
		     $qry="select distinct exam_date_time from exam_notifications";
			 $rslt=mysqli_query($mysql,$qry);
			 if($rslt)
			 {
				 while($ro=mysqli_fetch_assoc($rslt))
	...
	<?php
		...
		and result_tb.enroll_id='$enroll'";
